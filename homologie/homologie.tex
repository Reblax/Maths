...
\begin{proof}
    Suppose $F$ is additive. $\id_0$ is the zero morphism of $\Hom_\cat (0,0)$. Therefore $F(\id_0) = \id_{F(0)}$ is the zero morphism of $\Hom_\catt (F(0),F(0))$. For any $Y \in \Ob(\catt)$ and $f : F(0) \to Y$, $f = f\id_{F(0)} = 0$. This shows $F(0)$ initial. A similar reasoning shows it is final. Therefore $F(0)$ is isomorphic to the zero object of $\catt$. \todo
\end{proof}
