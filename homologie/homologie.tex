...
\begin{proof}
    ...
    \begin{center}
        ...
        \end{tikzcd}
    \end{center}
    We need to recover a map $G(X) \to G(Y)$ such that composing with it gives us $\gamma$. This works by the Yoneda lemma, which tells us that the natural transformation $\gamma$ comes from an element $\Hom(G(X),G(Y))$. Call it $G(f)$. Using this diagram with $X=Y$ and $f = \id_X$ shows that $G(\id_X)= \id_{G(X)}$. It remains to check this does define a functor. Let $X \xrightarrow{f} Y \xrightarrow{g} Z$ in $\cat$. Then we have the diagram
    \begin{center}
        \begin{tikzcd}
            \Hom(G(Z),-)
                \arrow[r,"- \circ G(g)"]
                \arrow[rr,bend left=10,"- \circ G(g\circ f)",start anchor=north,end anchor=north,shorten=2mm]
                \arrow[d,"\sim" sloped]
            &
            \Hom(G(Y),-)
                \arrow[r,"- \circ G(f)"]
            ...
            \Hom(Z,F(-))
                \arrow[r,"- \circ g"]
                \arrow[rr,bend right=10,"- \circ (g \circ f)",',start anchor=south,end anchor=south,shorten=2mm]
            &
            \Hom(Y,F(-))
                \arrow[r,"- \circ f"]
            &
        ...
        \end{tikzcd}
    \end{center}
    Which shows $G(g\circ f) = G(g) \circ G(f)$ (because the map $\gamma$ above is unique).
\end{proof}
...
\begin{examples} \leavevmode
    \begin{enumerate}
        ...
        \item If $\Jcat$ is a small category, there is a map $\Delta : \Ob(\cat) \to \Fun(\Jcat,\cat)$ where $\Delta(c)$ is the constant functor at $c$, that is the functor that sends all objects to $c$ and all morphisms to $\id_c$.
    \end{enumerate}
\end{examples}
...
Concretely, a limit of $F : \Jcat \to \cat$ is a pair $(\lim F, \phi)$ with $\lim F \in \Ob(\cat)$ and $\phi : \Delta(\lim F) \Rightarrow F$ is such that for any \todo
