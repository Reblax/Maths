...
\begin{definition}
    Let $F : \cat \to \cat'$ be a functor between two $k$-linear categories. The functor $F$ is said to be \emph{$k$-linear} (or \emph{additive} when $k = \Z$) if for any $X,Y \in \Ob(\cat)$, $\applic{\Hom(X,Y)}{\Hom(FX,FY)}{f}{F(f)}$ is a $k$-linear map.
\end{definition}

\begin{proposition}
    A functor $F : \cat \to \cat'$ is additive if and only if $F(0) \simeq 0$ and $F(X \oplus Y) \simeq F(X) \oplus F(Y)$.
\end{proposition}
